...
export default function DuplicatePage() {
  ...
  async function duplicate() {
    ...
    const result = await table.create({
        fields: {
          ...
          CP_2025_d_AE_d_annees_precedentes: previsionnel.CP_2025_d_AE_d_annees_precedentes,
          Date: new Date(),
    ...
    const additions = await window.grist.getTable("Depenses_previsionnelles").create(toCreate)
    setWIP(false)
  }
  ...
  return (
    <div className="fullPage">
      {
        previsionnel ?
        <button disabled={wip} onClick={duplicate}>{actionTitle()}</button>
       :
